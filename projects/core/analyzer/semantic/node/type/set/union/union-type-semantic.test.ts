...
test('a is integer or float', () => {
  ...
  expect(typeSemantic.$).toBe($Node.UnionTypeSemantic);
...
test('1 check type', () => {
  ...
  expect(aType.$).toBe($Node.IdType);
  expect(bType.$).toBe($Node.UnionTypeSemantic);
...
test('2 check type', () => {
  ...
  expect(aType.$).toBe($Node.UnionTypeSemantic);
