...
//   expect(constNode.id?.getText().toNativeString()).toBe('a');
...
//     ? (typeNodeType(semantic, constNode.annotation) as StringSemantic)
//     : nothing;
//   expect(typeSemantic?.$.toNativeString()).toBe($StringSemantic().toNativeString());
...
//   expect(typeSemantic?.$).toBe($StringSemantic());
//   expect(typeSemantic?.value.toNativeString()).toBe('abc');
// });
