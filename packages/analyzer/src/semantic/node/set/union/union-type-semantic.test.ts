...
test('a is integer or float', () => {
  ...
  expect(typeSemantic?.left.$).toBe($UsageSemantic());
  expect((typeSemantic?.left as UsageSemantic).declaration?.name.toNativeString()).toBe('Integer');
  expect(typeSemantic?.right.$).toBe($UsageSemantic());
  expect((typeSemantic?.right as UsageSemantic).declaration?.name.toNativeString()).toBe('Float');
});
...
test('1 check type', () => {
  ...
  expect(aType?.$.toNativeString()).toBe($UsageSemantic().toNativeString());
