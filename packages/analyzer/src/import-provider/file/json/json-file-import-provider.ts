...
export type JsonFileImportProvider = FileImportProvider & Brand<'Analyzer.JsonFileImportProvider'>;

export const $JsonFileImportProvider = () =>
  $AnalyzerType<JsonFileImportProvider>('JsonFileImportProvider', $ImportProvider());

export function newJsonFileImportProvider(): JsonFileImportProvider {
  return {
    $: $JsonFileImportProvider(),
...
function getDeclarationsFromUri(uri: Uri): DeclarationScope {
  ...
  const declarationScope = newDeclarationScope();

  return jsonToObjectTypeSemantic(uri);
}

function jsonToObjectTypeSemantic(uri: Uri): DeclarationScope {
  const buffer = readFileSync(uri.value.toNativeString());
  ...
  const scope = newDeclarationScope();
