...
describe('group node', () => {
  ...
  test('Single element', () => {
    ...
    expect(
      ((node.items.first()?.node as ExpressionStatementNode).expression as IdNode).getText().toNativeString(),
    ).toBe('a');
  });
  ...
  test('Inner group', () => {
    ...
    expect(
      ((node.items.first()?.node as ExpressionStatementNode).expression as GroupNode).items.count(),
    ).toBe(0);
  });
