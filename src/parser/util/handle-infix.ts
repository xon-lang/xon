...
export function handleInfix(context: ParserContext, operator: OperatorNode, left: Node, right: Node | null): Node {
  ...
  if (operator.text === ASSIGN_TOKEN) {
    return createAssign(context, operator, left, right);
  }

  return infixNode(context, operator, left, right);
}
...
function createAssign(context: ParserContext, operator: OperatorNode, left: Node, right: Node | null): Node {
  const assign = assignNode(context, operator, right);

  if (is<IdNode>(left, NodeType.ID)) {
    return idAssignNode(left, assign);
  }

  if (is<ArrayNode>(left, NodeType.ARRAY)) {
    return arrayAssignNode(left, assign);
  }

  if (is<ObjectNode>(left, NodeType.OBJECT)) {
    return objectAssignNode(left, assign);
  }

  if (is<GroupNode>(left, NodeType.GROUP)) {
