...
test('model animal', () => {
  ...
  const tree = parseDeclaration(code) as DefinitionDeclarationTree;

  expect(tree).toBeInstanceOf(DefinitionDeclarationTree);
...
test('model animal with only attribute', () => {
  ...
  const attributes = tree.attributes.map(
    (x) => (x as DeclarationStatementTree).declaration as ParameterDeclarationTree,
  );
...
test('model cat with generics', () => {
  ...
  expect(tree.generics.length).toBe(1);
  expect(tree.generics[0].name?.text).toBe('T');
  expect((tree.generics[0].type?.expression as IdExpressionTree).name.text).toBe('Number');
