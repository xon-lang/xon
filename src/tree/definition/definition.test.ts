...
test('one scope', () => {
  ...
  expect(tree.abstract.name).toBe('BaseClass');
  expect(tree.abstract.generics.length).toBe(2);
  expect(tree.abstract.generics[0].name).toBe('String');
  expect(tree.abstract.generics[1].name).toBe('Boolean');
