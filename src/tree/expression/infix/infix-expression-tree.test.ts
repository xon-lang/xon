...
test('several operands with different priorities', () => {
  ...
  expect((left.expression as OperatorExpressionTree).name.text).toBe('+');
...
test('several operators', () => {
  ...
  expect(tree).toBeInstanceOf(InfixExpressionTree);
  expect(tree.left).toBeInstanceOf(IntegerExpressionTree);
  expect(tree.operator.text).toBe('/');

  expect(tree.right).toBeInstanceOf(PrefixExpressionTree);
  expect((tree.right as PrefixExpressionTree).operator.text).toBe('+');

  expect((tree.right as PrefixExpressionTree).expression).toBeInstanceOf(IntegerExpressionTree);
  expect(((tree.right as PrefixExpressionTree).expression as IntegerExpressionTree).value).toBe(2);
});
