...
//   const definition = getTypeDefinition(left.type);
...
//   if (foundMethod) {
//     return (foundMethod.type as FunctionTypeTree).returnType;
//   }

//   throw IssueService.instance.addError(
//     left,
//     `Wrong operation "${left.type.toString()} ${operator} ${right.type.toString()}"`,
//     `Add "${operator}" operator in "${left
//       .type
//       .toString()}" with "${right.type.toString()}" parameter`,
//   );
// };
