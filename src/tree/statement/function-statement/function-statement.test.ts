...
test('function with one expression body', () => {
  ...
  const tree = parseStatement<FunctionStatementTree>(code);
  ...
  expect((tree.value.args[0].value as LiteralExpressionTree).literal.value).toBe('simple string');

  const type2 = tree.value.args[1].type as SimpleTypeTree;
  expect(type2.name).toBe('i8');
  expect(tree.value.args[1].name).toBe('num');
  expect((tree.value.args[1].value as LiteralExpressionTree).literal.value).toBe(7);
