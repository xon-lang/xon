...
import { IntegerLiteralExpressionTree } from './integer-literal-expression.tree';

test('positive number', () => {
    const code = '144126';
    const tree = parseCode(code, IntegerLiteralExpressionTree);
...
test('no underscore at the start', () => {
    const code = '_123';
    parseWrongCode(code, IntegerLiteralExpressionTree);
});
