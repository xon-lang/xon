...
export const evalInfixExpression = (tree: InfixExpressionTree, argsMap = {}): unknown => {
...
export const evalPrefixExpression = (tree: PrefixExpressionTree, argsMap = {}): unknown => {
  const value = evalExpression(tree.value, argsMap);
  return eval(`${tree.operator}${escapeIfString(value)}`);
};

export const evalPostfixExpression = (tree: PostfixExpressionTree, argsMap = {}): unknown => {
  const value = evalExpression(tree.value, argsMap);
