...
test('has items', () => {
  ...
  expect(metadata.items[0].equals(new LiteralTypeMetadata(1, scope.core.integer))).toBe(true);
  expect(metadata.items[1].equals(new LiteralTypeMetadata('hi', scope.core.string))).toBe(true);
});
