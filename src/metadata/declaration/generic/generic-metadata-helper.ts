...
export function getShadowParameterMetadata(
  ...
  const metadata = new GenericMetadata(tree.name || tree, scope);
  const innerScope = scope.create();

  if (tree.destructure.length) {
    for (const parameter of tree.destructure) {
      parameter.metadata = new GenericMetadata(parameter.name, scope);
      if (parameter.name) {
    ...
    metadata.parameters = tree.parameters.map((x) => x.metadata);
  }
